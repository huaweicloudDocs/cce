...
-   已创建VPC和子网，并获取VPC和子网的ID，具体请参见[创建VPC](创建VPC.md)。
...
2.  调用[创建集群](创建集群.md)接口创建一个不带节点的空集群。
    ...
    ```
    curl -k -H "X-Auth-Token:$Token" -H "Content-Type:application/json;charset=utf-8" -X POST https://{cce_endpoint}/api/v3/projects/{project_id}/clusters -d '{
        "kind": "Cluster",
        "apiVersion": "v3",
        "metadata": {
            "name": "cluster-sample"
...
3.  集群创建成功后，调用[创建节点](创建节点.md)接口为该集群创建一个节点。
    ...
    ```
    curl -k -H "X-Auth-Token:$Token" -H "Content-Type:application/json;charset=utf-8" -X POST https://{cce_endpoint}/api/v3/projects/{project_id}/clusters/{cluster_id}/nodes -d '{
        "kind": "Node",
        "apiVersion": "v3",
        "metadata": {
            "name": "node-sample"
