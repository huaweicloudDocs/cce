# 购买鲲鹏集群（即将下线）<a name="cce_01_0121"></a>

云容器引擎CCE在国内首家发布了基于鲲鹏系列芯片的容器服务，鲲鹏容器集群（ARM指令集）提供了容器在鲲鹏（ARM架构）服务器上的运行能力，提供与X86服务器相同的调度伸缩，快速部署能力，并具有大幅降低成本的潜力。

>![](public_sys-resources/icon-notice.gif) **须知：** 
>-   鲲鹏集群在新版控制台中不再支持创建，如您有使用ARM架构节点的需求，您可在1.19版本的CCE集群中添加鲲鹏节点。
>-   当前仅华北-北京四、华北-乌兰察布一、华东-上海一、华东-上海二、华南-广州、亚太-新加坡、非洲-约翰内斯堡区域支持鲲鹏集群。
...
3.  设置集群参数，其中带“\*”的参数需重点关注。
    ...
    **表 1**  创建集群参数配置
    ...
    <td class="cellrowborder" valign="top" width="79.97999999999999%" headers="mcps1.2.3.1.2 "><p id="p1363744211114"><a name="p1363744211114"></a><a name="p1363744211114"></a>该参数仅对开通企业项目的企业客户帐号显示。选择某企业项目（如：default）后，集群、集群下节点、集群安全组、节点安全组和自动创建的节点EIP（弹性公网IP）将创建到所选企业项目下。为方便管理资源，在集群创建成功后，建议不要修改集群下节点、集群安全组、节点安全组的企业项目。</p>
    ...
    <td class="cellrowborder" valign="top" width="79.97999999999999%" headers="mcps1.2.3.1.2 "><p id="p9100682161231"><a name="p9100682161231"></a><a name="p9100682161231"></a>Kubernetes社区基线版本，建议选择最新的版本。版本升级请参见<a href="集群升级概述-10.md">集群升级概述</a>。</p>
    ...
    <td class="cellrowborder" valign="top" width="79.97999999999999%" headers="mcps1.2.3.1.2 "><p id="p14899102111550"><a name="p14899102111550"></a><a name="p14899102111550"></a>集群管理规模是指当前集群的控制节点可以管理的最大工作节点规模，您可以选择50节点、200节点、1000节点三种管理规模，请根据您的业务需求选择，该规模在集群创建后不可更改，请慎重选择。</p>
    ...
    <td class="cellrowborder" valign="top" width="79.97999999999999%" headers="mcps1.2.3.1.2 "><p id="p8259153311912"><a name="p8259153311912"></a><a name="p8259153311912"></a>认证机制主要用于对集群下的资源做权限控制。</p>
    <p id="p1186713484551"><a name="p1186713484551"></a><a name="p1186713484551"></a>默认开启<strong id="b167461052192710"><a name="b167461052192710"></a><a name="b167461052192710"></a>X509认证模式</strong>，X509是一种非常通用的证书格式。</p>
...
4.  单击“下一步：创建节点“，在“创建节点“步骤中，参照如下参数配置节点。
    ...
    -   **节点规格：**请根据业务需求选择节点规格。
        ...
        -   AI加速型：AI加速型节点，搭载高性能、低功耗的海思Ascend 310 AI处理器，实现快速高效地处理推理和图像识别等工作，适用于图像识别、视频处理、推理计算以及机器学习等场景。
            ...
            >-   当前AI加速型节点仅在部分可用区可选。
            >-   选用AI加速型的节点前需要安装**huawei-npu**插件，以保证使用昇腾 310芯片资源的负载可以正常运行，点此[前往安装](huawei-npu-160.md)。
            >-   节点创建成功后会安装Ascend 310芯片驱动并自动触发节点重启，期间会有短暂的节点不可用，属于正常现象，重启完成后可恢复正常。


        **图 3**  选择节点规格<a name="fig662183511267"></a>  
        ![](figures/选择节点规格-12.png "选择节点规格-12")

        >![](public_sys-resources/icon-note.gif) **说明：** 
        >为确保节点稳定性，系统会自动预留部分资源，运行必须的系统组件。详细请参见[节点预留资源计算公式](节点预留资源计算公式-32.md)。
    ...
    -   **云服务器高级设置：**（可选），单击![](figures/zh-cn_image_0183134608.png)展开后可对节点进行如下高级功能配置：
        ...
        -   **子网IP：**可选择“自动分配IP地址“和“手动分配IP地址“，推荐使用“自动分配IP地址“。
            ...
            >![](public_sys-resources/icon-note.gif) **说明：** 
            >使用“手动分配IP地址“时，由于Master节点IP地址为随机指定，因此可能存在Node节点IP与Master节点IP地址冲突的情况。如需使用“手动分配IP地址“，建议在设置Node节点[所在子网](购买CCE集群-5.md#li796613104535)时，选择一个与Master节点不同的子网网段。
...
8.  待集群状态为“正常”，表示鲲鹏集群创建成功，集群名称前方显示为鲲鹏图标。

    ![](figures/zh-cn_image_0000001152476967.png)
...
-   单击已成功创建的集群名称，进入“集群详情“页可查看集群详情。
    ...
    **表 2**  已创建的集群详情
    ...
    <div class="note" id="cce_01_0028_note16137184513428"><a name="cce_01_0028_note16137184513428"></a><a name="cce_01_0028_note16137184513428"></a><span class="notetitle"> 说明： </span><div class="notebody"><p id="cce_01_0028_p1137134574211"><a name="cce_01_0028_p1137134574211"></a><a name="cce_01_0028_p1137134574211"></a>由于版本更新演进，旧版Console不再支持查看新集群的事件，请前往新版Console使用。</p>
    </div></div>
    </td>
    </tr>
    ...
    <p id="cce_01_0028_p218234511328"><a name="cce_01_0028_p218234511328"></a><a name="cce_01_0028_p218234511328"></a>v1.17版本的集群不再支持AOM提供的弹性伸缩机制，请使用节点池功能进行弹性伸缩，详情请参见<a href="节点池概述-35.md">节点池概述</a>。</p>
    </td>
    </tr>
