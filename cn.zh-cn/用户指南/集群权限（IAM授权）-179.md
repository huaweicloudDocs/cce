...
>集群权限仅针对与集群相关的资源（如集群、节点等）有效，您必须确保同时配置了[命名空间权限](命名空间权限（Kubernetes-RBAC授权）-180.md)，才能有操作Kubernetes资源（如工作负载、Service等）的权限。
...
![](figures/给用户授予CCE权限流程-131.png "给用户授予CCE权限流程-131")
...
IAM中预置的CCE系统角色为**CCE Administrator**，给用户组授予该系统角色权限时，必须同时授予该角色依赖的其他策略才会生效，例如Tenant Guest、Server Administrator、ELB Administrator等。了解更多角色依赖关系，请参考：[系统权限](https://support.huaweicloud.com/usermanual-permissions/iam_01_0001.html)。
