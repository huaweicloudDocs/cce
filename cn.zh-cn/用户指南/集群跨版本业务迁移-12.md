...
<td class="cellrowborder" valign="top" width="74.63%" headers="mcps1.2.3.1.2 "><a name="ol161155617140"></a><a name="ol161155617140"></a><ol id="ol161155617140"><li>确认应用中存储，是否使用云存储，或者自己搭建存储。</li><li>自动创建的存储需要在新集群中变成使用已有存储。</li></ol>
</td>
</tr>
...
1.  **创建新集群**
    ...
    创建与老版本集群同规格同配置的集群，创建方法请参见[购买CCE集群](购买CCE集群-5.md)。
...
3.  **创建存储**
    ...
    在新集群中使用已有存储创建PVC，PVC名称不变，方法请参见[存储卷声明PVC](存储卷声明PVC-92.md)。
...
4.  **创建工作负载**
    ...
    在新集群中创建工作负载，名称和规格参数保持不变，创建方法请参见[创建无状态负载\(Deployment\)](创建无状态负载(Deployment)-40.md)或[创建有状态负载\(StatefulSet\)](创建有状态负载(StatefulSet)-41.md)。在工作负载中挂载存储，方法请参见[部署带云硬盘存储卷EVS的工作负载](部署带云硬盘存储卷EVS的工作负载-99.md)。
...
7.  **老集群退订或删除**
    ...
    新集群全部功能ready，退订或者删除老集群，删除集群方法请参见[删除集群（按需计费）](删除集群（按需计费）-14.md)。
