...
-   CCE中的Windows集群和裸金属集群不支持创建对象存储服务。
...
3.  配置如[表1](#table20328123218464)基本信息。
    ...
    **表 1**  创建对象存储基本信息配置
    ...
    <td class="cellrowborder" valign="top" width="74%" headers="mcps1.2.3.1.2 "><p id="p30952037173155"><a name="p30952037173155"></a><a name="p30952037173155"></a>对象存储支持的类别：</p>
    ...
    <div class="notice" id="note10353370173230"><a name="note10353370173230"></a><a name="note10353370173230"></a><span class="noticetitle"> 注意： </span><div class="noticebody"><p id="p26071469173230"><a name="p26071469173230"></a><a name="p26071469173230"></a>低频访问存储有额外数据读取费用。 <a href="http://support.huaweicloud.com/pro_price/#obs_detail" target="_blank" rel="noopener noreferrer">了解对象存储服务计费详情</a></p>
    </div></div>
    </td>
    </tr>
    ...
    <td class="cellrowborder" valign="top" width="74%" headers="mcps1.2.3.1.2 "><p id="p1550119168137"><a name="p1550119168137"></a><a name="p1550119168137"></a>ReadWriteMany</p>
    </td>
    </tr>
    </tbody>
    </table>
...
1.  参照[创建无状态工作负载](创建无状态工作负载.md)或[创建有状态工作负载](创建有状态工作负载.md)创建工作负载，在添加容器后，展开“数据存储“，选择“云存储“页签中，单击“添加云存储“。
2.  选择存储类型为“对象存储“。
    ...
    **表 2**  对象存储参数设置
    ...
    <td class="cellrowborder" valign="top" width="79%" headers="mcps1.2.3.1.2 "><p id="p323192813419"><a name="p323192813419"></a><a name="p323192813419"></a>选择已创建的存储，您需要提前创建存储。</p>
    </td>
    </tr>
    ...
    <p id="p13630164485017"><a name="p13630164485017"></a><a name="p13630164485017"></a>对象存储子类型为标准存储、低频访问存储。</p>
    </td>
    </tr>
    ...
    **_vi pvc-obs-auto-example.yaml_**
    ...
    metadata: 
      annotations: 
        ...
        volume.beta.kubernetes.io/storage-provisioner: flexvolume-huawei.com/fuxiobs  # 必须为flexvolume-huawei.com/fuxiobs
    ...
    spec: 
      ...
      resources: 
        requests: 
          storage: 1Gi              # 存储容量，单位为Gi，对OBS桶来说，此处仅为校验需要（不能为空和0），设置的大小不起作用
    ```

    其中：

    -   “volume.beta.kubernetes.io/storage-class”为桶类型，当前支持标准（obs-standard）和低频（obs-standard-ia）两种桶。
    -   “name”为创建的PVC名称。
    -   “storage”为存储容量，单位为Gi，对OBS桶来说，此处仅为校验需要（不能为空和0），设置的大小不起作用。
    -   其中“volume.beta.kubernetes.io/storage-provisioner“必须使用“flexvolume-huawei.com/fuxiobs“。
...
## 使用kubectl挂载对象存储<a name="section83940541116"></a>
