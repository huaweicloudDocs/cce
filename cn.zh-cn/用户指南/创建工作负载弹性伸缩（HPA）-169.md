...
使用HPA需要安装能够提供Metrics API的插件，如metrics-server和Prometheus。

## 约束与限制<a name="section107429267459"></a>

-   HPA策略：仅支持1.13及以上版本的集群创建。
-   每个工作负载只能创建一个策略，即如果您创建了一个HPA策略，则不能再对其创建[CustomedHPA策略](创建工作负载弹性伸缩（CustomedHPA）-170.md)或其他HPA策略，您可以删除该HPA策略后再创建。
...
1.  在CCE控制台，单击左侧导航栏的“弹性伸缩“，在“工作负载伸缩“页签下，单击“创建HPA策略“。
