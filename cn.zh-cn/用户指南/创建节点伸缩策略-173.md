...
CCE的自动伸缩能力是通过节点自动伸缩组件[autoscaler](autoscaler-152.md)实现的，可以按需弹出节点实例，支持多可用区、多实例规格、多种伸缩模式，满足不同的节点伸缩场景。
...
1.  在CCE控制台，单击左侧导航栏的“弹性伸缩“，在“节点伸缩“页签下，单击“创建节点伸缩策略“按钮。
...
4.  在打开的“创建节点伸缩策略“页面中，参照如下参数设置策略。
    -   策略名称：新建策略的名称，请自定义。
    -   关联节点池：请单击“添加节点池“后选择要操作的节点池。您可以关联多个节点池，以使用相同的伸缩策略。
        ...
        >节点池优先级功能详情请参见[弹性扩缩容](创建节点池-36.md#li153215432027)。
    ...
    -   执行规则：单击“添加规则“，在弹出的添加规则窗口中设置如下参数：
        ...
        **规则类型：**可选择“指标触发“或“周期触发“，两种类型区别如下：
        ...
        -   **指标触发：**
            ...
            -   执行动作：与上述“触发条件“相对应，达到触发条件值后所要执行的动作。如[图1](#fig8972202013283)中所示，当内存分配率超过40%时将增加5个节点。

                **图 1**  指标触发-执行动作<a name="fig8972202013283"></a>  
                ![](figures/指标触发-执行动作.png "指标触发-执行动作")

        -   **周期触发：**
            -   触发时间：可选择每天、每周、每月或每年的具体时间点，如[图2](#fig997262013281)所示，则为每天15:00触发。

                **图 2**  周期触发时间<a name="fig997262013281"></a>  
                ![](figures/周期触发时间.png "周期触发时间")

            -   执行动作：与上述“触发时间“相对应，达到触发时间值后所要执行的动作。如[图3](#fig1697214203288)中所示，即每天15:00时将执行增加5个节点的动作。

                **图 3**  定时触发-执行动作<a name="fig1697214203288"></a>  
                ![](figures/定时触发-执行动作.png "定时触发-执行动作")


        您可以单击“添加规则“，设置多条节点伸缩策略。您最多可以添加1条CPU使用率指标规则、1条内存使用率指标规则，且规则总数小于等于10条。

5.  设置完成后，单击“创建“，在“完成“步骤中若显示“创建节点伸缩策略\*\*\*任务提交成功“，可单击“返回节点策略“。
6.  在“节点伸缩“页签下，可以看到刚创建的节点伸缩策略。
...
节点伸缩策略中不能直接设置缩容策略，您可以在安装[autoscaler插件](autoscaler-152.md)时进行设置。

节点缩容仅支持资源分配率缩容机制，当集群下CPU和内存分配率低于您设置的门限（在安装/编辑autoscaler插件时设置）时将对节点池下节点启动缩容（该功能可以关闭），如[图5](#fig9643195391116)。

**图 5**  自动缩容设置<a name="fig9643195391116"></a>  
![](figures/自动缩容设置-129.png "自动缩容设置-129")
