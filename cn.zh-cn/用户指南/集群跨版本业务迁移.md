...
3.  **创建存储**
    ...
    在新集群中创建存储，存储类型使用已有存储，PVC名称不变，创建方法请参见[使用云硬盘存储卷](使用云硬盘存储卷.md)。

4.  **创建工作负载**

    在新集群中创建工作负载，名称和规格参数保持不变，创建方法请参见[创建无状态负载\(Deployment\)](创建无状态负载(Deployment).md)或[创建有状态负载\(StatefulSet\)](创建有状态负载(StatefulSet).md)。
...
7.  **老集群退订或删除**
    ...
    新集群全部功能ready，退订或者删除老集群，删除集群方法请参见[删除集群](删除集群.md)。
