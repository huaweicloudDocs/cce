...
2.  参照[表2](#table8638121213265)设置新增集群参数，其中带“\*”的参数为必填参数。
    ...
    **表 2**  创建集群参数配置
    ...
    <td class="cellrowborder" valign="top" width="80%" headers="mcps1.2.3.1.2 "><a name="ul1095563315716"></a><a name="ul1095563315716"></a><ul id="ul1095563315716"><li>Underlay：传统单层网络，是当前数据中心网络基础转发架构的网络。</li><li>VPC网络：基于VPC网络的自定义路由，直接将容器网络承载于VPC网络之中。每个节点将会被分配固定大小的IP地址段。vpc-router网络由于没有隧道封装的消耗，容器网络性能相对于容器隧道网络有一定优势。vpc-router集群由于VPC路由中配置有容器网段与节点IP的路由，可以支持集群外直接访问容器实例等特殊场景。适用于高性能场景，可支持200节点规模。</li></ul>
    </td>
    </tr>
...
2.  （可选）若列表中没有裸金属服务器，请单击“新增裸金属服务器”购买。购买完成后再执行下一步。详细步骤请参见[创建裸金属服务器](https://support.huaweicloud.com/usermanual-bms/zh-cn_topic_0035100414.html)。
