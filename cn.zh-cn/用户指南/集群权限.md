...
-   示例1：创建一个名称为“test”的集群
    ...
    样例中的 CCE:\*:\*:cluster:test 对应如下：

    ```
    service-name:region:account-id:resource-type:resource-path
    ```

    -   **service-name：**IAM注册的服务名称，如云容器引擎服务即为**CCE**。
    -   **region：**Region名称，CCE各组件需要从环境变量中读取，上线前需确保Deployment环境变量中有HWS\_REGION值。
    -   **account-id：**用户账号ID（Domain-id）从上下文userInfo中获取。
    -   **resource-type：**各服务向IAM注册的资源类型，如CCE为**cluster**类型。
    -   **resource-path：**限定资源的路径，CCE当前限定为**集群名称**，支持设置模糊匹配，如test\*，表示集群名称开头为test的所有集群。
...
-   示例3：拒绝用户删除某类集群下的节点
    ...
    自定义策略支持限定资源的路径，CCE当前限定为**集群名称**，支持设置模糊匹配，如test\*，表示集群名称开头为test的所有集群。
...
-   由于历史原因，CCE的API中保留了对[应用管理与运维平台（ServiceStage）](https://support.huaweicloud.com/servicestage/index.html)的三个系统角色（SvcStg Administrator、SvcStg Developer、SvcStg Operator）的兼容，当前CCE和ServiceStage已经全面适配了IAM的细粒度策略进行权限管理，**不建议您继续使用该系统角色进行权限管理**。其在CCE侧的具体权限如下：
    ...
    -   SvcStg Administrator：拥有CCE Administrator相同权限，例外：拥有此角色的用户不默认拥有cluster-admin的命名空间权限（Kubernetes RBAC）。
    -   SvcStg Developer：拥有CCE Administrator相同权限，例外：拥有此角色的用户不默认拥有cluster-admin的命名空间权限（Kubernetes RBAC）。
    -   SvcStg Operator：拥有CCE的只读权限，但不包括集群的命名空间权限。
...
>-   Tenant Administrator、Tenant Guest是特殊的IAM系统角色，当配置任意系统或自定义策略后，TenantAdmin、TenantGuest将以系统策略形式生效，用于兼容IAM RBAC和ABAC场景。  
